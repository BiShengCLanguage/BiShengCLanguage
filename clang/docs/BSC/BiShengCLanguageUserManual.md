...
##### 3.1.4.5. `_Owned` 指针可空性对所有权规则的影响

对于 `_Owned _Nullable` 指针 `p`，二元的“拥有所有权”和“失去所有权”状态会拓展为以下状态：
* `p` 必定为空。此时 `p` 一定没有所有权。
* `p` 必定非空。此时 `p` 具有“拥有所有权”和“失去所有权”状态，与 `p` 是 `_Nonnull` 时一致。
* `p` 可能为空。此时 `p` 具有“拥有所有权”和“失去所有权”状态，对应于 `p` 非空时的状态。

指针的可空状态判定见 [非空指针章节](#34-非空指针)。所有权规则会以以下准则进行拓展：
1. 当 `p` 不是必定为空时，其允许的操作与 `p` 必定非空时一致（除了不允许潜在的空指针解引用）。
2. 当 `p` 必定为空时，其允许的操作与使用空指针能够安全进行的操作一致。

对于 `_Owned _Nullable` 指针 `p`，下文叙述规则时:
1. “拥有所有权”包含了(1)`p` 必定非空、拥有所有权，或(2) `p` 可能为空、若非空则拥有所有权的情况。
2. “失去所有权”包含了(1)`p` 必定非空、失去所有权，(2) `p` 可能为空、若非空则失去所有权，或(3) `p` 必定为空的情况。

注：本节所述的规则由所有权分析和[非空指针分析](#34-非空指针)组合实现。非空指针分析关闭时，对 `_Nullable` 指针的解引用和 `_Nullable` 到 `_Nonnull` 指针的赋值限制不会生效。

###### `_Nonnull` 与 `_Nullable` 非空/可能为空的情况

对于 `_Owned _Nonnull` 指针 `p`，适用以下基准规则：

以下操作要求 `p` 拥有所有权，且执行后会失去所有权：
1. 使用 `p` 对其他 `_Owned` 或 `_Owned _Nullable` 指针进行初始化：`T * _Owned p2 = p`
2. 使用 `p` 对其他 `_Owned` 或 `_Owned _Nullable` 指针（要求必定没有所有权）进行赋值：`p2 = p`
3. 作为函数调用的实参传给 `_Owned` 或 `_Owned _Nullable` 形参：`foo(p)`
4. 作为 `_Owned` 或 `_Owned _Nullable` 返回值被返回：`return p`
5. 使用 `__move_to_raw` 或 `__move_array_to_raw` 显式转移所有权给裸指针：`__move_to_raw(p)` / `__move_array_to_raw(p)`

以下操作要求 `p` 拥有所有权，但执行后不会失去所有权：
1. 解引用：`*p = 42`，`p->v = 42`
2. 取借用：`&_Mut *p`、`&_Const *p`

以下操作要求 `p` 不拥有所有权，且执行后会获得所有权：
1. 对 `p` 进行赋值：`p = ...`

以下操作对 `p` 的所有权状态没有要求，也不会改变所有权状态：
1. 对指针的比较操作（如 `p == p2`，`p == nullptr` 等）

`p` 在退出作用域时如果仍拥有所有权，则会编译报错（报内存泄露）。

对于 `_Owned _Nullable` 指针 `p`，如果可空分析判定为必定非空，则其规则在上述基准上有以下变化：

1. 对 `p` 进行赋值：要求 `p` 不拥有所有权，执行后 `p` 的状态由右侧表达式决定：
   - 右侧必定为空：`p` 必定为空，右侧状态不变。
   - 右侧必定非空、拥有所有权：`p` 必定非空、拥有所有权，右侧变为必定非空、失去所有权。
   - 右侧可能为空、若非空则拥有所有权：`p` 可能为空、若非空则拥有所有权，右侧变为可能为空、必定失去所有权。

对于 `_Owned _Nullable` 指针 `p`，如果可空分析判定为"可能为空"，则规则在“必定非空”的基础上有以下变化：
1. **解引用不允许**：`*p = 42`、`p->v = 42` 等均不允许
2. **转移所有权操作的接收方仅限 `_Owned _Nullable` 类型**：上述"执行后会失去所有权"的 5 项操作，接收方只能为 `_Owned _Nullable` 类型（不能为 `_Nonnull`）。

###### `_Owned _Nullable` 必定为空的情况

对于 `_Owned _Nullable` 指针 `p`，如果可空分析判定为“必定为空”，则适用以下规则：

以下操作允许，且执行后状态不变：
1. 使用 `p` 对其他 `_Owned _Nullable` 指针进行初始化：`T * _Owned _Nullable p2 = p`
2. 使用 `p` 对其他 `_Owned _Nullable` 指针（要求必定没有所有权）进行赋值：`p2 = p`
3. 作为函数调用的实参传给 `_Owned _Nullable` 形参：`foo(p)`
4. 作为 `_Owned _Nullable` 返回值被返回：`return p`
5. 使用 `__move_to_raw` 或 `__move_array_to_raw`：`__move_to_raw(p)` 或 `__move_array_to_raw(p)`，返回空指针

解引用不允许：`*p = 42`，`p->v = 42`。

取借用允许：`&_Mut *p`、`&_Const *p`，结果必定为空。

允许对 `p` 进行赋值 `p = ...`，执行后根据等号右侧决定状态：
- 右侧必定为空：`p` 必定为空，右侧状态不变。
- 右侧必定非空、拥有所有权：`p` 必定非空、拥有所有权，右侧变为必定非空、失去所有权。
- 右侧可能为空、若非空则拥有所有权：`p` 可能为空、若非空则拥有所有权，右侧变为可能为空、必定失去所有权。
...
毕昇 C 的**借用是一个指针类型，指向了借用对象存储的内存地址**。为表达借用的概念：
1. 引入新关键字`_Borrow`，用`_Borrow`来修饰指针类型 T*，表示 T 的借用类型。`_ArrayElem`也可以与`_Borrow`一并修饰指针类型 T*, 表示对 T 数组成员的借用类型。
2. 引入借用操作符 `& _Mut` 和 `& _Const`，其中，`& _Mut e`表示获取对表达式 e 的**可变借用**，`& _Const e`表示获取对表达式`e`的**只读借用**。此处要求表达式`e`是 lvalue（左值）与标准 C 中的取地址操作符`&`类似, 借用操作符实际上获取的是表达式`e`的地址，借用表达式`&_Mut e`和`&_Const e`的结果的值类别是 rvalue（右值）。
...
我们可以对不同种类的对象取借用：`_Owned`指针、局部变量、全局变量、参数等，或是结构体的某个成员等。为了避免取借用得到的指针悬垂、导致程序使用本不该使用的数据，我们引入生命周期的概念来表示借用左值表达式和借用对象的有效作用域，并对两者的生命周期做检查。3.2.2的余下章节将详细介绍生命周期的具体规则。
...
为了避免悬垂指针的问题，毕昇 C 规定：任何一个对资源的借用，都不能比资源的所有者的生命周期长。也就是说：借用左值表达式的生命周期，不能比借用对象的生命周期长。在这个例子中，对`*p`的解引用要求`p`的生命周期覆盖`*p = 3`这一行，而借用对象`local2`的生命周期在上一行作用域结束时就结束了。因此毕昇C通过检查借用左值表达式和借用对象的生命周期，在编译期就可以识别出这样的潜在安全风险。

##### 3.2.2.2. 借用左值表达式和借用对象

每个借用左值表达式（也就是 _Borrow 指针类型的左值表达式）都会有一个或多个借用对象，由以下规则决定：

1. 对于取借用操作(`&_Mut e`, `&_Const e`)得到的借用指针，除规则 2-5 所列的情形外，借用对象即为`e`。
2. 对于`&_Mut *p`, `&_Const *p`（`p`为`_Borrow`指针），借用对象即为`p`的借用对象。
3. 对于`&_Mut *p`, `&_Const *p`（`p`为`_Owned`指针或裸指针），借用对象为`p`所指向的对象。
4. 对于 `&_Mut arr[i]` / `&_Const arr[i]`（`arr` 为数组 lvalue），借用对象为 `arr[]`（整个数组）。
5. 对于 `&_Mut p[i]` / `&_Const p[i]`（`p`为`_Owned _ArrayElem`指针，`_Borrow _ArrayElem`指针，或者裸指针），借用对象为`p`所指向的对象所在的整个数组。
6. 如果在不同分支、循环语句中，借用指针有不同的借用对象，则在这些分支、循环语句结束后的执行路径交汇处，借用指针的借用对象是每条执行路径上最后的借用对象集合的并集。
7. 对于借用指针类型的函数形参，借用对象为调用点传递过来的借用对象。
8. 对于借用指针类型的函数返回值，该函数的形参中应有一个或多个借用类型的参数，返回值的借用对象是所有借用类型参数的借用对象的并集。
9. 如果借用左值表达式`e1`被另一个借用左值表达式`e2`借用（`e1`和`*e2`互为别名），对`*e2`赋值（如`*e2 = r`），则`*e2`的借用对象是原来`*e2`的借用对象和`r`的借用对象的并集合，并且也会更新`e1`的借用对象。
...
void foo(int a, int *_Owned b, int *c, struct S d) {
  // 借用对象是普通局部变量
  int local = 5;
  int *_Borrow p1 = &_Mut local; // p1的借用对象是local
  int *_Borrow p2 = &_Mut * p1;  // p2的借用对象是local
  int *_Borrow p3 = p1;         // p3的借用对象是local

  // 借用对象是owned变量
  int *_Owned x1 = safe_malloc<int>(2);
  int *_Borrow p4 = &_Mut * x1; // p4的借用对象是*x1

  // 借用对象是裸指针变量
  int *x2 = malloc(sizeof(int));
  int *_Borrow p5 = &_Mut * x2; // p5的借用对象是*x2

  // 借用对象是结构体的某个字段
  struct S s = {.a = 5};
  int *_Borrow p6 = &_Mut s.a; // p6的借用对象是s.a

  // 借用对象是函数的返回值，与被调用函数的借用类型入参的 “借用对象” 一样
  int local1 = 10, local2 = 20;
  // 被调用函数bar有两个借用类型入参，因此p7的借用对象是local1和local2
  int *_Borrow p7 = bar(&_Mut local1, &_Mut local2);

  // 借用对象是全局变量
  const int *_Borrow p8 = &_Const g; // p8的借用对象是g

  // 借用对象是函数入参
  int *_Borrow p9 = &_Mut a;    // p9的借用对象是a
  int *_Borrow p10 = &_Mut * b; // p10的借用对象是*b
  int *_Borrow p11 = &_Mut * c; // p11的借用对象是*c
  int *_Borrow p12 = &_Mut d.a; // p12的借用对象是d.a

  // 借用对象是整个数组 arr[]
  int arr[8] = {0};
  int *_Borrow _ArrayElem p14 = &_Mut arr[2]; // p14 的借用对象是 arr[]

  // 分支、循环语句后执行路径交汇处，借用对象取并集
  {
    int local3 = 1;
    int local4 = 2;
    int *_Borrow p13;
    if (a > 0) {
      p13 = &_Mut local3; // p13的借用对象是local3
    } else {
      p13 = &_Mut local4; // p13的借用对象是local4
    }
    *p13 = 3; // p13的借用对象是local3和local4
  }
...
void baz(void) {
  int a = 1, b = 2;
  int *_Borrow p = &_Mut a; // p的借用对象是a
  int *_Borrow *_Borrow q = &_Mut p; // q的借用对象是p
  *q = &_Mut b; // p和*q的借用对象是a和b
}
...
一个变量的生命周期从它的声明开始，到当前整个语句块结束，这个设计被称为 Lexical Lifetime，因为变量的生命周期是严格和词法中的作用域范围绑定的。对于不是借用左值表达式的表达式（尤其是作为借用对象时），其生命周期是 Lexical Lifetime。我们给出具体的定义：
...
| 局部字面量 | "string literal" | 整个程序，从程序开始到退出，一直存在 |
...
void f1() {
  int a = 1;
  int *p = &a;
  int *_Borrow p1 = (int *_Borrow)(int *)(&_Mut *p); // 不分析p1的借用对象(*p)
  int *_Borrow p2 = &_Mut *&a; // 不分析p2的借用对象(a)
}
```

对于借用左值表达式，Lexical Lifetime 过于保守了，某些情况下借用左值表达式的作用范围被过度拉长了，以至于某些实质上是安全的代码也被阻止了，这在一定程度上限制了程序员能编写出的代码。因此，毕昇 C 为借用左值表达式引入 Non-Lexical Lifetime（简写为NLL），用更精细的手段计算借用左值表达式真正起作用的范围。

借用左值表达式`e`的 NLL 范围取以下两者更长的那个：
1. 从借用处开始，一直持续到最后一次使用的地方；
2. 从借用处开始，且如果借用左值表达式`e`呗另一个借用左值表达式`e1`借用（如`e1 = &_Mut e`或`e1 = &_Const e`），则`e`的 NLL 持续到`e1`的 NLL 结束处。

如果借用左值表达式是一个标识符（如`p`），则`p`在被赋值后会开始一个新的生命周期并结束旧的生命周期。

其中，以下场景属于对借用左值表达式 `p` 的使用：
1. 函数调用，如 `use(p)`
2. 函数返回 `return p`
3. 解引用 `*p`
4. 成员访问 `p->field`
5. 赋值 `q = p`、取借用`&_Mut p`/`&_Const p`/`&_Mut *p`/`&_Const *p`、类型转换
...
// 本例中p的NLL是分段的，每段NLL都有一个借用对象
void foo() {
  int local1 = 1, local2 = 2;  //#1
  int *_Borrow p = &_Mut local1; //#2，p的第一段NLL开始，借用对象为local1
  other_op();                  //#3
  use(p);                      //#4，p的第一段NLL结束
  other_op();                  //#5
  p = &_Mut local2;             //#6，p的第二段NLL开始，借用对象为local2，由于后面没有再对p的使用，p的NLL结束
  other_op();     //#7
}
...
对于借用，我们有这样的生命周期约束：**借用左值表达式的生命周期，不能比借用对象的生命周期长**。如果一个借用有多个借用对象，取生命周期最短的那个进行比较。
...
// 本例中，p的生命周期为[2,4]，借用对象local的生命周期为[1,4]，满足生命周期约束
void test1() {
  int local = 5;              //#1
...
// 本例中，p 的生命周期有两段：
// ok: 第一段为 [2,2]，借用对象 local1 的生命周期为 [1,8]，满足生命周期约束
// error: 第二段为 [5,7]，借用对象 local2 的生命周期为 [4,6]，不满足生命周期约束
void test2() {
  int local1 = 5;              //#1
...
// 本例中p的生命周期有两段：
// ok: 第一段为[2,2]，借用对象local1的生命周期为[1, 8]，满足生命周期约束
// error: 第二段为[5,7]，借用对象有两个，分别是local1和local2，其中local2的生命周期为[4, 6]，不满足生命周期约束
void test3() {
  int local1 = 5;                       //#1
  int *_Borrow p = &_Mut local1;          //#2
  {                                     //#3
...
// error: use(p)时，p的借用对象local2的生命周期已经结束，不满足生命周期约束
// ok: else 分支满足生命周期约束
void test4() {
  int local = 5;              //#1
  int *_Borrow p = &_Mut local; //#2
...
//本例中，p的生命周期为[2,4]，借用对象*x的生命周期为[1,3]，不满足生命周期约束，error
void test5() {
...
毕昇 C 将借用指针的权限进行了分级，分为可变（mut）借用和不可变（immut）借用，我们可以通过操纵可变借用指针来读写借用对象的内容，通过不可变借用指针，我们只能读取借用对象的内容，但是不能修改它。例如：
```C
// 可变借用指针类型为 T *_Borrow
void use_mut(int *_Borrow p) {
  // 通过可变借用指针，可以修改借用对象的值
...
void use_immut(const int *_Borrow p) {
  *p = 5; // error: 无法通过不可变借用指针，来修改借用对象的值
  // 通过不可变借用指针，可以读取借用对象的值
  int a = *p;
}
...
void test1() {
  int local = 1;
  int *_Borrow p1 = &_Mut local;
  int *_Borrow p2 = &_Mut local; // error: 同一时刻最多只能有一个指向local的可变借用左值表达式
  write(p1);
  write(p2);
}
...
如果同时存在对一个变量的可变借用和不可变借用，可能会出现通过可变借用修改借用对象的内存状态，然后再使用不可变借用访问被修改的内存，从而导致未定义行为的情况。
例如：
...
由于不可变借用不会导致借用对象被修改，因此同一时刻可以拥有任意多个不可变借用，例如：
```C
void read(const int *_Borrow p) {}
...
##### 3.2.4.1. 不可变借用对借用对象的影响

对表达式 e 做不可变借用， 即`&_Const e`，表达式 e 进入 "只读" 状态。在这个不可变借用的生命周期结束之前，e 只能读不能修改，也不能被取可变借用。如果 e 是 _Borrow 指针类型，则 *e 也进入只读状态。如果 e 是结构体类型，则 e 的所有成员都进入只读状态。如果 e 是数组类型，则数组的所有元素都进入只读状态。
...
##### 3.2.4.2. 可变借用对借用对象的影响

对表达式 e 做可变借用， 即`&_Mut e`，表达式 e 进入 "冻结" 状态。在这个可变借用的生命周期结束之前，e 不能读，不能修改（包含被move），也不能被借用。如果 e 是 _Borrow 指针类型，则 *e 也进入冻结状态。如果 e 是结构体类型，则 e 的所有成员都进入冻结状态。如果 e 是数组类型，则数组的所有元素都被冻结。
...
2. 如果函数参数中有一个借用类型的参数，函数返回是借用类型，那么我们直接认为这个返回值的借用，是来自于这个借用类型参数，返回的借用的 “借用对象” 与这个借用类型参数的 “借用对象” 一样，这个返回的借用也应该满足前面提到的那些借用规则。

3. 如果函数参数中有多个借用类型的参数，函数返回是借用类型，那么我们直接认为这个返回值的借用，其“借用对象”是所有借用类型参数的 “借用对象”的并集，这个返回的借用也应该满足前面提到的那些借用规则。

4. 在函数定义内，对于借用指针类型的形参，其借用对象为调用点传递过来的“借用对象”，这些对象的生命周期覆盖整个函数。

5. 在函数定义内，对于函数形参中的借用类型形参 pa1、pa2，如果 pa1、pa2 任意一个是多级借用类型，则禁止`pa1`、`*pa1`（包括与他们别名的局部变量）等与`pa2`、`*pa2`（包括与他们别名的局部变量）等之间赋值。
...
int *_Borrow *_Borrow f3(int *_Borrow *_Borrow p); // error

void f4(int *_Borrow *_Borrow p, int *_Borrow *_Borrow q) {
  p = q; // error
  *p = *q; // error
}
...
void test() {
  ...
  /* 函数 f1 的参数创建了一个对 local 的可变借用，这个借用被传递给了返回值 p1，
     导致 p1 相当于是对 local 的一个可变借用, 所以返回值 p1 的借用对象是
  ...
  /* 函数 f2 的参数创建了一个对 local1 和 local2
     ...
     local2 的一个可变借用, 所以返回值 p2 的借用对象是 local1 和 local2, 在 p2
...
如果结构体包含借用类型的成员，那么毕昇C会为该结构体整体计算 NLL。访问结构体的成员时，如果该成员是借用类型，那么它的生命周期是整个结构体的生命周期。该结构体的借用对象是所有借用类型成员的借用对象的并集，该结构体的生命周期不能比任意一个借用对象的生命周期长。
...
对于 `T * _Borrow` 或 `const T * _Borrow` 类型的借用左值表达式 `e`，其解引用 `*e` 的类型为 `T`。
...
7. 不可变借用指针直接或间接指向的借用指针，如果中间不经过裸指针，则都被视作不可变借用。
...
1. 在安全区内，对全局变量与静态变量只允许取只读借用，不允许取可变借用。这是为了保证安全性，因为对于全局变量与静态变量，我们无法在函数签名中跟踪哪个函数读取了它们，哪个函数修改了它们。
...
2. 借用左值表达式在使用前必须初始化。详细规则在[3.7. 初始化分析](#37-初始化分析)章节中介绍。
```C
void test() {
  int *_Borrow p; 
  use(p); // error: 必须初始化
...
3. 不能声明全局变量与静态变量为借用类型。
```C
#include "bishengc_safety.hbs" // BiShengC 语言提供的头文件，用于安全地进行内存分配及释放

int g = 5;
int *_Borrow p = &_Mut g; // error: 借用左值表达式不允许是全局变量
...
4. 不允许对函数取借用，不允许 `_Borrow` 修饰函数指针类型。
...
10.  允许对借用类型使用一元的`&`（取地址、获得裸指针）、`&_Const`（取不可变借用）、`&_Mut`（取可变借用）、`!`（判断指针是否为空）及二元的`&&`（逻辑与）、`||`（逻辑或）运算符。

11.  不允许对借用类型使用一元的`-`（取负数）、`~`（按位取反），也不允许对借用类型使用二元的`&`（按位与）、`|`（按位或）、`<<`（左移）、`>>`（右移）运算符。
...
对于类型上包含数组大小的数组 `a`，以下条件全部满足的 `for` 循环为合格 `for` 循环：
...
能够使用合格 `for` 循环改变数组元素所有权的前提是数组的类型中包含数组大小。所有定长数组、VLA 都满足该条件。但是对于结构体中的柔性数组成员（flexible array member）：

```c
struct S {
  int len;
  int *_Owned arr[]; // arr 是柔性数组成员
};
```

其在数组类型中没有大小，同类型的不同对象也可以有不同的大小。对于这样的数组，编译器无法验证某个 `for` 循环是否满足“循环上界等于数组长度”的要求。因此，柔性数组成员不会有合格 `for` 循环，不能用循环改变数组元素所有权。
...
当前非空指针检查不处理指针存在别名的情况。对于安全指针间由取借用或 reborrow 带来的别名则有相应的处理规则保证安全。这在安全区可以保证不存在空指针解引用，但是在非安全区、涉及裸指针间（或裸指针与安全指针之间）存在别名的情况，非空指针检查无法保证不存在空指针解引用。若想在非安全区追求避免空指针解引用，请尽可能避免引入指针别名，以免漏检。

```c
void f(int ** p) {
  int ** p2 = p; // p2 与 p 别名
  if (p && *p && p2) {
    *p2 = nullptr; // 通过 p2 修改 *p
    **p = 10; // 空指针解引用，但是检查不报错
  }
}
int main(void) {
